@extends('layouts.app', ['title' => 'รายงานข้อมูลการผ่าชันสูตรศพ'])
...
<div class="card report-page">
        ...
        <form method="GET" class="report-filter-form">
            ...
            @php
                $tabs = [
                    ...
                    'doctor' => ['label' => 'แพทย์', 'class' => 'tab-doctor'],
                    'method' => ['label' => 'รูปแบบการผ่า', 'class' => 'tab-type'],
                    'assistant' => ['label' => 'ผู้ช่วยผ่า', 'class' => 'tab-assistant'],
                    'photo' => ['label' => 'ผู้ช่วยถ่ายภาพ', 'class' => 'tab-police'],
                    'lab' => ['label' => 'Lab', 'class' => 'tab-type'],
    ...
    <section class="report-summary">
        ...
        <div class="summary-card draft">
            ...
            <div class="summary-value">{{ number_format($summary['pending']) }}</div>
        </div>

        <div class="summary-card submitted">
            <div class="summary-label">อนุมัติแล้ว</div>
            <div class="summary-value">{{ number_format($summary['approve']) }}</div>
        </div>
    </section>
    ...
    <section class="report-panel">
        <div class="report-panel-header">
            <div>
                <div class="report-kicker">รายงานกราฟ Autopsy</div>
        ...
        <div class="chart-box">
            <canvas id="autopsyChart"></canvas>
        </div>
    </section>

    <section class="report-panel">
        <div class="report-panel-header">
            <div class="report-title">รายการข้อมูลการผ่าชันสูตรศพ</div>
        </div>
        ...
        <div class="report-table-wrap">
            <table class="report-table">
                <thead>
                    <tr>
                        <th>หมายเลขผ่าชันสูตร</th>
                        <th>วันที่ผ่า</th>
                        <th>แพทย์</th>
                        <th>รูปแบบการผ่า</th>
                        <th>ผู้ช่วยผ่า</th>
                        <th>ผู้ช่วยถ่ายภาพ</th>
                        <th>Lab</th>
                        <th>สถานะ</th>
                    </tr>
                </thead>
                ...
                <tbody>
                    @forelse ($cases as $case)
                        <tr>
                            <td>
                                <a href="{{ route('autopsy-cases.show', $case) }}" class="report-link">
                                    {{ $case->autopsy_no }}
                                </a>
                            </td>
                            <td>{{ optional($case->autopsy_date)->format('d/m/Y') }}</td>
                            <td>{{ $case->doctor?->displayName() ?: '-' }}</td>
                            <td>{{ $case->autopsy_method === 'autopsy' ? 'ผ่า' : 'ไม่ผ่า' }}</td>
                            <td>{{ $case->assistant?->name ?: '-' }}</td>
                            <td>{{ $case->photo?->name ?: '-' }}</td>
                            <td>{{ $case->lab?->name ?: '-' }}</td>
                            <td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const ctx = document.getElementById('autopsyChart');
    if (!ctx) return;
